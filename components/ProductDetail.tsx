  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
      ...
      <div className="flex flex-col gap-3">
        <div className={`relative w-full rounded-[1.4rem] overflow-hidden bg-[#F5EFE4] ${isCamisas ? 'aspect-[3/4]' : 'aspect-square'}`}>
          {images[activeImage] && (
            <Image
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-none" style={{ scrollbarWidth: 'none' }}>
            {images.map((img, i) => (
              <button
                ...
                className={`relative shrink-0 rounded-[0.7rem] overflow-hidden border-2 transition-all duration-200 ${isCamisas ? 'w-[54px] h-[72px]' : 'w-[72px] h-[72px]'} ${i === activeImage ? 'border-red' : 'border-transparent opacity-60 hover:opacity-100'}`}
              >
